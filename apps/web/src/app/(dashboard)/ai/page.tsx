...
import { Sparkles, ChevronDown, Check, X, Loader2, Zap } from 'lucide-react';
...
const AI_ACTIONS: ActionDef[] = [
  {
    key: 'summary',
    label: 'Summarize',
    description: 'Generate a delivery-risk–aware summary',
    emoji: '📋',
  ...
  },
  {
    ...
    description: 'Auto-propose actionable subtasks',
    emoji: '🔀',
  ...
  },
  {
    ...
    description: 'Analyze impact and recommend priority',
    emoji: '🎯',
  ...
  },
  {
    key: 'comment',
    label: 'Draft Comment',
    description: 'Generate a professional status update',
    emoji: '💬',
...
export default function AIPage() {
  ...
  async function runAction(actionKey: string) {
    ...
    try {
      ...
      setResult({ actionKey, text: 'Failed to fetch AI suggestion. Please try again.' });
    } finally {
      setLoading(null);
    }
  }
    ...
    <div className="space-y-5">
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-slate-400 mt-0.5">
              {selectedProject?.name ?? ''} · {selectedIssue?.status?.replace('_', ' ') ?? ''}
            </p>
          </div>
          <ChevronDown
            size={16}
          ...
          <div className="mt-1 rounded-2xl bg-white border border-slate-200 shadow-xl overflow-hidden max-h-60 overflow-y-auto">
            {allIssues.map((issue) => {
              ...
              return (
                <button
                  key={issue.id}
                  onClick={() => { setSelectedIssueId(issue.id); setShowPicker(false); setResult(null); }}
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-slate-400 mt-0.5">{proj?.name} · {issue.status.replace('_', ' ')}</p>
                  </div>
          ...
          {AI_ACTIONS.map((action) => {
            ...
            return (
              <button
                ...
                className={`bg-gradient-to-br ${action.gradient} border ${action.accentBorder} rounded-2xl p-4 text-left transition active:scale-[0.97] disabled:opacity-50`}
              >
      ...
      {result && resultAction && (
        <div className="rounded-2xl bg-white border border-indigo-200 shadow-sm overflow-hidden">
          {/* Result header */}
          <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-indigo-50 to-violet-50 border-b border-indigo-100">
            <div className="flex items-center gap-2">
              <Sparkles size={14} className="text-indigo-600" />
              <span className="text-[13px] font-bold text-indigo-800">
                {resultAction.emoji} {resultAction.label}
              </span>
            </div>
          ...
          <div className="px-4 py-4">
            <p className="text-[14px] text-slate-700 whitespace-pre-wrap leading-relaxed">{result.text}</p>
          </div>
          ...
          <div className="flex border-t border-slate-100 divide-x divide-slate-100">
            <button
              ...
              className="flex-1 py-3 text-sm text-slate-500 hover:bg-slate-50 transition font-medium"
            >
              Dismiss
            </button>
            <button className="flex-1 py-3 text-sm font-semibold text-indigo-600 hover:bg-indigo-50 transition">
              Apply to Issue
            </button>
          </div>
        </div>
